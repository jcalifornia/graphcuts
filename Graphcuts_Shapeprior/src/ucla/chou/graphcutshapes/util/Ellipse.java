 ...
 * @version $Id: Ellipse.java,v 1.2 2007/04/15 21:02:22 hbitteur Exp $
 */
public class Ellipse
{
     ...
     */
    public Ellipse (Vector<Integer> x,
                   Vector<Integer> y)
    {
        fit(x, y);
    ...
    protected void computeAngleAndAxes ()
    {
        if (Math.abs(B) < EPSILON) {
            ...
            if ((B / P) <= (-B / P)) {
                major = Math.sqrt(2 / ((A + C) + (B / P)));
                minor = Math.sqrt(2 / ((A + C) - (B / P)));
            } else {
                // Switch
                major = Math.sqrt(2 / ((A + C) - (B / P)));
                minor = Math.sqrt(2 / ((A + C) + (B / P)));
     ...
     */
    protected void fit (Vector<Integer> x,
                        Vector<Integer> y)
    {
        ...
        for (int i = 0; i < nbPoints; i++) {
            final double tx = x.get(i);
            final double ty = y.get(i);
            D1.set(i, 0, tx * tx);
            D1.set(i, 1, tx * ty);
            D1.set(i, 2, ty * ty);
            D2.set(i, 0, tx);
